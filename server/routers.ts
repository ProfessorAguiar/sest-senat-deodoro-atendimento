...
import { createCopilotConversation, postCopilotActivity, pollCopilotActivities, waitForCopilotReply, generateDeodoroAgentResponse } from "./copilotService";
import { clearTestSession, issueTestSession, verifyTestPassword, isTestAccount, getTestUserFromRequest } from "./testAuth";
...
export const appRouter = router({
  ...
  auth: router({
    ...
    internalMe: publicProcedure.query(async ({ ctx }) => getTestUserFromRequest(ctx.req)),
  ...
  copilot: router({
    ...
    sendMessage: publicProcedure.input(z.object({ conversationId: z.string(), text: z.string(), history: z.array(z.object({ role: z.enum(["user", "assistant"]), content: z.string() })).optional(), clientName: z.string().optional(), companyName: z.string().optional(), isCompany: z.boolean().optional() })).mutation(async ({ input }) => { const directLinePosted = await postCopilotActivity(input.conversationId, input.text); const copilotReply = directLinePosted ? await waitForCopilotReply(input.conversationId) : null; const reply = copilotReply || await generateDeodoroAgentResponse({ userMessage: input.text, history: input.history || [], clientName: input.clientName, companyName: input.companyName, isCompany: input.isCompany }); return { directLinePosted, copilotReplyReceived: Boolean(copilotReply), reply, timestamp: new Date().toISOString() }; }),
  ...
  lead: router({
    ...
    recent: protectedProcedure.query(async ({ ctx }) => { const saved = await listLeadRecordsDb(); return saved.length ? saved.slice(0, 30) : listLeadRecords().slice(0, 30); }),
